...
import { allWorks, allAuthors } from 'contentlayer/generated'
import type { Authors, Work } from 'contentlayer/generated'
...
export default async function Page(props: { params: Promise<{ slug: string[] }> }) {
  const params = await props.params
  const slug = decodeURI(params.slug.join('/'))
  const sortedCoreContents = allCoreContent(sortPosts(allWorks))
  ...
  const post = allWorks.find((p) => p.slug === slug) as Work
  const authorList = post?.authors || ['default']
  const authorDetails = authorList.map((author) => {
    const authorResults = allAuthors.find((p) => p.slug === author)
    return coreContent(authorResults as Authors)
